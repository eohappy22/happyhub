...
-- Força Priority ativo sempre ao iniciar
...
local function kickSelf()
    local ok = pcall(function()
        if game.Shutdown then
            ...
            LocalPlayer:Kick("\nHAPPY HUB PRIVATE - xi loves you <3")
        end
    end)
    if not ok then
        pcall(function()
...
local Theme = {
    ...
    SurfaceHighlight= Color3.fromRGB(35, 35, 35),
...
ShowNotification = function(title, text)
    ...
    f.BackgroundColor3 = Color3.fromRGB(28, 14, 52)
    ...
    TweenService:Create(f,      fadeIn, {BackgroundTransparency = 0.25}):Play()
        ...
        for _, v in pairs(Lighting:GetChildren()) do
            if v:IsA("BloomEffect") or v:IsA("BlurEffect") or v:IsA("ColorCorrectionEffect") or 
               v:IsA("SunRaysEffect") or v:IsA("DepthOfFieldEffect") or v:IsA("Atmosphere") then
                if v.Name ~= "PR1V_DarkMode_CC" then
                    v:Destroy()
                end
            end
        end
...
function setDarkMode(enabled)
    ...
    if on then
        ...
        local cc = Lighting:FindFirstChild("PR1V_DarkMode_CC")
        if not cc then
            cc = Instance.new("ColorCorrectionEffect")
            cc.Name = "PR1V_DarkMode_CC"
            cc.Parent = Lighting
        end
        cc.Brightness = 0.03
        cc.Contrast = 0.04
...
task.spawn(function()
    ...
    -- ══════════════════════════════════════════════════════════════
    -- AUTO STEAL GUI — layout limpo, fiel ao screenshot
    -- ══════════════════════════════════════════════════════════════
    ...
    local PANEL_W = IS_MOBILE and 260 or 310
    local PANEL_H = IS_MOBILE and 420 or 500
    local TAB_H   = 44  -- altura da barra de abas na base
    ...
    frame.BackgroundColor3 = Color3.fromRGB(32, 16, 60)
    ...
    ApplyViewportUIScale(frame, PANEL_W, PANEL_H, 0.5, 0.9)
    ...
    if IS_MOBILE then
        ...
        menuToggleBtn.Size = UDim2.new(0, 74, 0, 28)
        menuToggleBtn.Position = UDim2.new(1, -80, 0.5, -14)
    ...
    -- ── Botão Auto Kick (topo do painel) ────────────────────────
    local autoKickBar = Instance.new("Frame", frame)
    autoKickBar.Name = "AutoKickBar"
    autoKickBar.Size = UDim2.new(1, -24, 0, 32)
    autoKickBar.Position = UDim2.new(0, 12, 0, 48)
    autoKickBar.BackgroundColor3 = Color3.fromRGB(40, 20, 70)
    autoKickBar.BackgroundTransparency = 0.3
    autoKickBar.BorderSizePixel = 0
    Instance.new("UICorner", autoKickBar).CornerRadius = UDim.new(0, 8)
    local akStroke = Instance.new("UIStroke", autoKickBar)
    akStroke.Color = Theme.Accent1
    akStroke.Thickness = 1
    akStroke.Transparency = 0.55

    local autoKickLabel = Instance.new("TextLabel", autoKickBar)
    autoKickLabel.Size = UDim2.new(1, -70, 1, 0)
    autoKickLabel.Position = UDim2.new(0, 10, 0, 0)
    autoKickLabel.BackgroundTransparency = 1
    autoKickLabel.Text = "AUTO KICK"
    autoKickLabel.Font = Enum.Font.GothamBold
    autoKickLabel.TextSize = 11
    autoKickLabel.TextColor3 = Theme.TextSecondary
    autoKickLabel.TextXAlignment = Enum.TextXAlignment.Left

    local autoKickToggleBtn = Instance.new("TextButton", autoKickBar)
    autoKickToggleBtn.Size = UDim2.new(0, 54, 0, 22)
    autoKickToggleBtn.Position = UDim2.new(1, -60, 0.5, -11)
    autoKickToggleBtn.BorderSizePixel = 0
    autoKickToggleBtn.Font = Enum.Font.GothamBold
    autoKickToggleBtn.TextSize = 11
    Instance.new("UICorner", autoKickToggleBtn).CornerRadius = UDim.new(0, 6)

    local function refreshAutoKickBtn()
        local on = Config.AutoKickOnSteal == true
        autoKickToggleBtn.Text = on and "ON" or "OFF"
        autoKickToggleBtn.BackgroundColor3 = on and Theme.Success or Theme.SurfaceHighlight
        autoKickToggleBtn.TextColor3 = on and Color3.new(0,0,0) or Theme.TextPrimary
    end
    refreshAutoKickBtn()

    autoKickToggleBtn.MouseButton1Click:Connect(function()
        Config.AutoKickOnSteal = not Config.AutoKickOnSteal
        refreshAutoKickBtn()
        if SharedState.SettingsAutoKickSet then
            SharedState.SettingsAutoKickSet(Config.AutoKickOnSteal)
        end
        SaveConfig()
        ShowNotification("AUTO KICK", Config.AutoKickOnSteal and "ATIVADO" or "DESATIVADO")
    end)

    -- ── Painel "CURRENT TARGET" ─────────────────────────────────
    local targetPanel = Instance.new("Frame", frame)
    targetPanel.Name = "TargetPanel"
    targetPanel.Size = UDim2.new(1, -24, 0, 62)
    targetPanel.Position = UDim2.new(0, 12, 0, 86)
    targetPanel.BackgroundColor3 = Color3.fromRGB(50, 28, 88)
    ...
    targetHeader.Size = UDim2.new(1, -14, 0, 16)
    targetHeader.Position = UDim2.new(0, 10, 0, 8)
    ...
    targetLabel.Size = UDim2.new(1, -14, 0, 24)
    ...
    progressBarFill.BackgroundColor3 = Theme.Accent1
    progressBarFill.BorderSizePixel = 0
    CreateGradient(progressBarFill)
    Instance.new("UICorner", progressBarFill).CornerRadius = UDim.new(0, 2)

    -- ── Label "AVAILABLE BRAINROTS" ─────────────────────────────
    local selectLabel = Instance.new("TextLabel", frame)
    selectLabel.Size = UDim2.new(1, -24, 0, 18)
    selectLabel.Position = UDim2.new(0, 12, 0, 156)
    selectLabel.BackgroundTransparency = 1
    selectLabel.Text = "AVAILABLE BRAINROTS"
    selectLabel.Font = Enum.Font.GothamBold
    selectLabel.TextSize = 10
    selectLabel.TextColor3 = Theme.TextSecondary
    selectLabel.TextXAlignment = Enum.TextXAlignment.Left

    -- ── Lista de brainrots (ScrollingFrame) ──────────────────────
    -- Ocupa do topo da lista até a barra de abas
    local listFrame = Instance.new("ScrollingFrame", frame)
    listFrame.Name = "PetList"
    listFrame.Size = UDim2.new(1, -24, 1, -(176 + TAB_H + 4))
    listFrame.Position = UDim2.new(0, 12, 0, 176)
    ...
    -- ── Barra de abas NEAREST / HIGHEST / PRIORITY ──────────────
    local tabBarBg = Instance.new("Frame", frame)
    tabBarBg.Name = "TabBar"
    tabBarBg.Size = UDim2.new(1, 0, 0, TAB_H)
    tabBarBg.Position = UDim2.new(0, 0, 1, -TAB_H)
    tabBarBg.BackgroundColor3 = Color3.fromRGB(20, 10, 42)
    tabBarBg.BackgroundTransparency = 0.0
    tabBarBg.BorderSizePixel = 0
    Instance.new("UICorner", tabBarBg).CornerRadius = UDim.new(0, 14)

    -- linha separadora no topo da barra
    local tabSepLine = Instance.new("Frame", tabBarBg)
    tabSepLine.Size = UDim2.new(1, 0, 0, 1)
    tabSepLine.Position = UDim2.new(0, 0, 0, 0)
    tabSepLine.BackgroundColor3 = Theme.Accent1
    tabSepLine.BackgroundTransparency = 0.55
    tabSepLine.BorderSizePixel = 0

    -- três botões lado a lado
    local TAB_NAMES = {"NEAREST", "HIGHEST", "PRIORITY"}
    local tabButtons = {}

    for idx, name in ipairs(TAB_NAMES) do
        local btn = Instance.new("TextButton", tabBarBg)
        btn.Name = "Tab_" .. name
        btn.Size = UDim2.new(1/3, 0, 1, 0)
        btn.Position = UDim2.new((idx-1)/3, 0, 0, 0)
        btn.BackgroundTransparency = 1
        btn.BorderSizePixel = 0
        btn.Text = name
        btn.Font = Enum.Font.GothamBold
        btn.TextSize = 13
        btn.TextColor3 = Color3.fromRGB(130, 110, 170)
        btn.AutoButtonColor = false

        -- divisor vertical (não para o primeiro)
        if idx > 1 then
            local div = Instance.new("Frame", tabBarBg)
            div.Size = UDim2.new(0, 1, 0.55, 0)
            div.Position = UDim2.new((idx-1)/3, 0, 0.225, 0)
            div.BackgroundColor3 = Theme.Accent1
            div.BackgroundTransparency = 0.6
            div.BorderSizePixel = 0
        end

        -- indicador ativo (linha embaixo do texto)
        local indic = Instance.new("Frame", btn)
        indic.Name = "Indic"
        indic.Size = UDim2.new(0.55, 0, 0, 2)
        indic.Position = UDim2.new(0.225, 0, 1, -3)
        indic.BackgroundColor3 = Theme.Accent1
    ...
    local tabNearest  = tabButtons["NEAREST"]
    local tabHighest  = tabButtons["HIGHEST"]
    local tabPriority = tabButtons["PRIORITY"]

    local function refreshTabStyles()
        local states = {
            NEAREST  = stealNearestEnabled,
            HIGHEST  = stealHighestEnabled,
            PRIORITY = stealPriorityEnabled,
        }
        for name, btn in pairs(tabButtons) do
            local active = states[name]
            btn.TextColor3 = active and Color3.new(1, 1, 1) or Color3.fromRGB(130, 110, 170)
            local ind = btn:FindFirstChild("Indic")
            if ind then ind.BackgroundTransparency = active and 0 or 1 end
        end
    end
    -- ────────────────────────────────────────────────────────────

    -- Botões fake para manter compatibilidade com updateStealPanelUI
    local _dummyFrame = Instance.new("Frame"); _dummyFrame.BackgroundTransparency = 1
    local function _makeDummy()
        local b = Instance.new("TextButton", _dummyFrame); b.BackgroundTransparency=1; b.Text=""; return b
    end
    local btnStealHighest  = _makeDummy()
    local btnStealPriority = _makeDummy()
    local btnStealNearest  = _makeDummy()
    local btnAutoTurret    = _makeDummy()
    local btnAutoKick      = _makeDummy()

    local toggleBtnContainer = Instance.new("Frame", frame)
    toggleBtnContainer.Size = UDim2.new(1,-30,0,0); toggleBtnContainer.Position = UDim2.new(0,15,1,-90)
    ...
    local function setStealMode(mode)
        ...
        if stealNearestEnabled or stealHighestEnabled or stealPriorityEnabled then
            ...
            SharedState.ManualTargetEnabled = false
        end
        Config.StealNearest = stealNearestEnabled
        Config.StealHighest = stealHighestEnabled
        Config.StealPriority = stealPriorityEnabled
        ...
        if SharedState.ListNeedsRedraw then
            ...
            if allPets and #allPets > 0 then
                for i = 1, #allPets do
                    ...
                    bStroke.Color = Theme.Accent1; bStroke.Thickness = 1.5; bStroke.Transparency = 1
                    ...
                    -- Badge de rank
                    local rankBadge = Instance.new("Frame", btn)
                    rankBadge.Size = UDim2.new(0,28,0,28)
                    rankBadge.Position = UDim2.new(0,6,0.5,-14)
                    rankBadge.BackgroundColor3 = barCol
                    rankBadge.BackgroundTransparency = 0.75
                    rankBadge.BorderSizePixel = 0
                    Instance.new("UICorner", rankBadge).CornerRadius = UDim.new(0,6)

                    local rankLabel = Instance.new("TextLabel", rankBadge)
                    ...
                    infoLabel.Position = UDim2.new(0,40,0,3)
                    ...
                    -- Valor em destaque (canto direito)
                    local valueLabel = Instance.new("TextLabel", btn)
                    valueLabel.Size = UDim2.new(0,70,0,28)
                    valueLabel.Position = UDim2.new(1,-74,0.5,-14)
                    valueLabel.BackgroundColor3 = barCol
                    ...
                    -- itemBar dummy para compatibilidade
                    local itemBar = Instance.new("Frame", btn)
                    itemBar.Size = UDim2.new(0,0,0,0); itemBar.BackgroundTransparency = 1

                    petButtons[i] = {button=btn, stroke=bStroke, rank=rankLabel, info=infoLabel, bar=itemBar, badge=rankBadge, value=valueLabel}
                    
                    btn.MouseButton1Click:Connect(function()
                        selectedTargetIndex = i
                        selectedTargetUID = petData.uid 
                        manualTargetEnabled = true
                        SharedState.ManualTargetEnabled = true
    ...
    -- Conectar abas modernas de modo de steal
    tabNearest.MouseButton1Click:Connect(function()
        setStealMode("nearest")
        SharedState.ListNeedsRedraw = false
        updateUI(autoStealEnabled, get_all_pets())
    end)

    tabHighest.MouseButton1Click:Connect(function()
        setStealMode("highest")
        SharedState.ListNeedsRedraw = false
        updateUI(autoStealEnabled, get_all_pets())
    end)

    tabPriority.MouseButton1Click:Connect(function()
        setStealMode("priority")
        SharedState.ListNeedsRedraw = false
        updateUI(autoStealEnabled, get_all_pets())
    end)

    -- Manter compatibilidade dos dummies (sem fazer nada visivel)
    btnStealNearest.MouseButton1Click:Connect(function() end)
    btnStealHighest.MouseButton1Click:Connect(function() end)
    btnStealPriority.MouseButton1Click:Connect(function() end)
    btnAutoTurret.MouseButton1Click:Connect(function() end)
    btnAutoKick.MouseButton1Click:Connect(function() end)
    ...
    task.spawn(function()
        while true do
            task.wait(2) -- aumentado de 1s para 2s: menos pressao no loop
    ...
    -- ============================================================
    -- STEALING BASE ESP — sinaliza a base de quem está roubando
    -- ============================================================
    local stealingESPHighlights = {}
    local stealingESPBillboards = {}

    local function clearStealingESP(plotName)
        if stealingESPHighlights[plotName] then
            if stealingESPHighlights[plotName].Parent then
                stealingESPHighlights[plotName]:Destroy()
            end
            stealingESPHighlights[plotName] = nil
        end
        if stealingESPBillboards[plotName] then
            if stealingESPBillboards[plotName].Parent then
                stealingESPBillboards[plotName]:Destroy()
            end
            stealingESPBillboards[plotName] = nil
        end
    end

    local function clearAllStealingESP()
        for plotName in pairs(stealingESPHighlights) do clearStealingESP(plotName) end
    end

    local function createStealingESPMarker(plot, sign, stealerName, brainrotNameText)
        local plotName = plot.Name
        -- Já existe? Atualiza texto se necessário
        if stealingESPBillboards[plotName] then
            local bb = stealingESPBillboards[plotName]
            local frame = bb:FindFirstChildOfClass("Frame")
            if frame then
                local nameLbl = frame:FindFirstChild("StealerNameLbl")
                local petLbl  = frame:FindFirstChild("PetNameLbl")
                if nameLbl then nameLbl.Text = "⚔ " .. (stealerName or "?") end
                if petLbl  then petLbl.Text  = "🎯 " .. (brainrotNameText or "UNKNOWN") end
            end
            return
        end

        -- Highlight roxo na base (cor da UI)
        local hl = Instance.new("Highlight")
        hl.Name = "StealingESPHighlight"
        hl.FillColor           = Theme.Accent1    -- roxo primario da UI
        hl.OutlineColor        = Theme.Accent2    -- roxo claro da UI
        hl.FillTransparency    = 0.60             -- transparente
        hl.OutlineTransparency = 0.20
        hl.Adornee = plot
        hl.Parent  = plot
        stealingESPHighlights[plotName] = hl

        -- BillboardGui flutuando acima do PlotSign
        local adornTarget = sign:FindFirstChildWhichIsA("BasePart") or sign
        local bb = Instance.new("BillboardGui")
        bb.Name = "StealingESPMarker"
        ...
        stealingESPBillboards[plotName] = bb

        local frame = Instance.new("Frame", bb)
        frame.Size = UDim2.new(1, 0, 1, 0)
        frame.BackgroundColor3 = Theme.Background  -- fundo roxo escuro da UI
        frame.BackgroundTransparency = 0.35        -- semi-transparente
        frame.BorderSizePixel = 0
        Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 8)

        local stroke = Instance.new("UIStroke", frame)
        stroke.Color = Theme.Accent1               -- roxo da UI
        ...
        iconLbl.TextSize = 26
        iconLbl.TextColor3 = Theme.Accent2         -- roxo claro da UI
        iconLbl.TextXAlignment = Enum.TextXAlignment.Center
        iconLbl.TextYAlignment = Enum.TextYAlignment.Center
        iconLbl.TextStrokeTransparency = 0.2
        iconLbl.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)

        local textArea = Instance.new("Frame", frame)
        textArea.Size = UDim2.new(1, -36, 1, 0)
        textArea.Position = UDim2.new(0, 34, 0, 0)
        textArea.BackgroundTransparency = 1

        local nameLbl = Instance.new("TextLabel", textArea)
        nameLbl.Name = "StealerNameLbl"
        nameLbl.Size = UDim2.new(1, -6, 0.5, 0)
        nameLbl.Position = UDim2.new(0, 0, 0, 0)
        nameLbl.BackgroundTransparency = 1
        nameLbl.Text = "⚔ " .. (stealerName or "?")
        nameLbl.Font = Enum.Font.GothamBlack
        nameLbl.TextSize = 13
        nameLbl.TextColor3 = Theme.Accent2         -- roxo claro da UI
        nameLbl.TextXAlignment = Enum.TextXAlignment.Left
        nameLbl.TextTruncate = Enum.TextTruncate.AtEnd
        nameLbl.TextStrokeTransparency = 0.3
        nameLbl.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)

        local petLbl = Instance.new("TextLabel", textArea)
        petLbl.Name = "PetNameLbl"
        petLbl.Size = UDim2.new(1, -6, 0.5, 0)
        petLbl.Position = UDim2.new(0, 0, 0.5, 0)
        petLbl.BackgroundTransparency = 1
        petLbl.Text = "🎯 " .. (brainrotNameText or "UNKNOWN")
        petLbl.Font = Enum.Font.GothamBold
        petLbl.TextSize = 11
        petLbl.TextColor3 = Theme.TextPrimary
        petLbl.TextXAlignment = Enum.TextXAlignment.Left
        petLbl.TextTruncate = Enum.TextTruncate.AtEnd
        petLbl.TextStrokeTransparency = 0.5
        petLbl.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
    end

    -- Encontra o plot de um jogador via Synchronizer
    local function findPlayerPlot(targetPlayer)
        local Plots = Workspace:FindFirstChild("Plots")
        ...
        for _, plot in ipairs(Plots:GetChildren()) do
            ...
            if ok and ch then
                ...
                if owner then
                    ...
                    if ownerId == targetPlayer.UserId then
                        local sign = plot:FindFirstChild("PlotSign")
                        return plot, sign
                    end
                end
    ...
    -- Loop principal do StealingESP (otimizado: uma passagem so por todos os plots)
    task.spawn(function()
        while true do
            task.wait(0.8)
            -- Coleta IDs de jogadores roubando e o brainrot
            local stealingPlayers = {}
            for _, plr in ipairs(Players:GetPlayers()) do
                if plr ~= LocalPlayer and plr:GetAttribute("Stealing") then
                    stealingPlayers[plr.UserId] = plr:GetAttribute("StealingIndex") or "?"
                end
            end
            -- Se ninguem esta roubando, limpa tudo e pula
            if not next(stealingPlayers) then
                clearAllStealingESP()
                continue
            end
            -- Uma unica iteracao pelos plots para mapear owner->plot
            local activePlots = {}
            local Plots = Workspace:FindFirstChild("Plots")
            if Plots then
                for _, plot in ipairs(Plots:GetChildren()) do
                    ...
                    if ok and ch then
                        ...
                        if owner then
                            local ownerId = (typeof(owner) == "Instance" and owner:IsA("Player")) and owner.UserId
                                         or (type(owner) == "table" and owner.UserId)
                            if ownerId and stealingPlayers[ownerId] then
                                local sign = plot:FindFirstChild("PlotSign")
                                if sign then
                                    local plr = Players:GetPlayerByUserId(ownerId)
                                    local displayName = plr and (plr.DisplayName or plr.Name) or "?"
                                    activePlots[plot.Name] = true
                                    createStealingESPMarker(plot, sign, displayName, stealingPlayers[ownerId])
                                end
                            end
                        end
                    end
            ...
            -- Remove marcadores de plots sem ladrao ativo
            local toRemove = {}
            for plotName in pairs(stealingESPBillboards) do
                if not activePlots[plotName] then
                    table.insert(toRemove, plotName)
                end
            end
            for _, plotName in ipairs(toRemove) do
                clearStealingESP(plotName)
            end
        end
    end)
    -- ============================================================
    -- FIM STEALING BASE ESP
    -- ============================================================
...
task.spawn(function()
    ...
    local function createAdminToolsPanel()
        ...
        toolsFrame.BackgroundTransparency = 0.38
        ...
        proxSliderBg.BackgroundColor3 = Color3.fromRGB(42, 22, 72)
    ...
    listContainer.BackgroundColor3 = Theme.Background; listContainer.BackgroundTransparency = 0.38
...
isOnCooldown = function(cmd)
    ...
    local function createPlayerRow(plr)
        ...
        row.BackgroundColor3 = Color3.fromRGB(42, 22, 72)
        ...
        -- status updated by shared 2s ticker below (see SharedState.AdminRowUpdateQueue)
        if not SharedState.AdminRowUpdateQueue then SharedState.AdminRowUpdateQueue = {} end
        table.insert(SharedState.AdminRowUpdateQueue, updateStatus)
    ...
    -- Ticker compartilhado para status das linhas do painel admin (evita N threads paralelas)
    task.spawn(function()
        while true do
            task.wait(2)
            if SharedState.AdminRowUpdateQueue then
                local q = SharedState.AdminRowUpdateQueue
                local i = 1
                while i <= #q do
                    local fn = q[i]
...
-- ── BULLY REMASTERED: Funções auxiliares de teleporte ─────────────────────
...
-- ── Funções auxiliares de teleport (lógica do haze) ─────────────────────────
...
-- ── Sistema de teleporte estável (baseado no script de referência) ───────────
-- Tabela de delays por nível de velocidade (1=Fastest, 2=Fast, 3=Normal, 4=Safe)
-- O nível 3 é o comportamento original (baseline).
runAutoSnipe = function()
    ...
    if SharedState.ManualTargetEnabled and SharedState.SelectedPetData then
        -- Brainrot selecionado manualmente na lista tem prioridade absoluta
...
local ok, err = pcall(function()
    ...
    task.wait(0.01)
    if not hrp or not hrp.Parent then error("hrp lost during jump start") end

    hrp.CFrame = CFrame.new(-409, 23, hrp.Position.Z)
    task.wait(0.1)
    if not hrp or not hrp.Parent then error("hrp lost after reposition") end
    ...
    waitUntilHeartbeat(function()
        return hrp and hrp.Parent
            ...
            and hum and hum.FloorMaterial ~= Enum.Material.Air
    end, 3.0)
    ...
    -- ── Fase final: vai DIRETO ao brainrot ───────────────────────────────────
    local targetPart2 = findAdorneeGlobal(targetPetData) or targetPart
    local petPos      = targetPart2.Position
    local verticalDiff = petPos.Y - hrp.Position.Y

    if verticalDiff > 2 then
        -- 2º andar: cria plataforma temporária perto do brainrot para não cair
        local airPos = Vector3.new(petPos.X, petPos.Y - 8, petPos.Z)
        ...
        -- Remove plataforma assim que o roubo começar (ou em 20s)
        task.spawn(function()
            local start = tick()
        ...
        -- 1º andar: teleporta diretamente em cima do brainrot
        for i = 1, 7 do
            if LocalPlayer:GetAttribute("Stealing") then break end
            hrp.AssemblyLinearVelocity = Vector3.new(
                hrp.AssemblyLinearVelocity.X, 0, hrp.AssemblyLinearVelocity.Z)
            if (hrp.Position - petPos).Magnitude > 3 then
    ...
    isTpMoving = State.isTpMoving
    if not ok then
        warn("[runAutoSnipe] erro: " .. tostring(err))
    end
end
-- ─────────────────────────────────────────────────────────────────────────────
...
local function executeReset()
    ...
    -- Garante respawn instantâneo (sem delay de spawn)
    pcall(function() Players.RespawnTime = 0 end)

    -- Desativa temporariamente o AntiDie do próprio hub para o reset funcionar
    local hadAntiDie = not (_G.AntiDieDisabled)
    if hadAntiDie then
    ...
    -- Mata o personagem instantaneamente
    pcall(function() humanoid.Health = 0 end)

    -- Restaura o AntiDie após o respawn
    if hadAntiDie then
        task.spawn(function()
...
do
    ...
    actionsFrame.BackgroundTransparency = 0.38
    ...
    actionsTitle.Text = "HAPPY HUB PRIVATE"
...
sFrame.BackgroundColor3 = Theme.Background; sFrame.BackgroundTransparency = 0.38
...
fovSliderBg.BackgroundColor3 = Color3.fromRGB(42, 22, 72)
...
task.spawn(function()
    ...
    ssFrame.BackgroundColor3 = Theme.Background; ssFrame.BackgroundTransparency = 0.38
...
task.spawn(function()
    ...
    task.spawn(function()
        while true do
            task.wait(0.5) -- aumentado de 0.3s para 0.5s
...
task.spawn(function()
    ...
    iFrame.BackgroundTransparency = 0.38
    ...
    local function CreateFancySlider(parent, name, min, max, default, callback)
        ...
        if IS_MOBILE then
            ...
            slideBg.BackgroundColor3 = Color3.fromRGB(60, 35, 95)
...
task.spawn(function()
    ...
    local HTheme = {
        Background = Color3.fromRGB(28, 14, 52),
        Accent1 = Color3.fromRGB(148, 103, 255),
        Accent2 = Color3.fromRGB(190, 155, 255),
        White   = Color3.fromRGB(235,235,245),
        Gray    = Color3.fromRGB(160, 140, 200),
    }
    ...
    main.BackgroundColor3 = Color3.fromRGB(28, 14, 52)
    ...
    brandCredit.Text = "@eohappy22 on tiktok"
...
task.spawn(function()
    ...
    availableScroll.BackgroundColor3 = Theme.Surface; availableScroll.BackgroundTransparency = 0.40
    ...
    priorityScroll.BackgroundColor3 = Theme.Surface; priorityScroll.BackgroundTransparency = 0.40
...
-- ============================================================
-- AUTO SELL MODULE (integrado ao PR1VHub)
-- ============================================================
task.spawn(function()

    -- Encontra o plot mais próximo do jogador
    ...
    -- Retorna os slots com ProximityPrompt de "sell" no plot mais próximo
    local function getSlots()
        ...
        for _, pod in pairs(podiums:GetChildren()) do
            ...
            if num then
                for _, v in pairs(pod:GetDescendants()) do
                    if v:IsA("ProximityPrompt") then
                        if v.ActionText and v.ActionText:lower():find("sell") then
                            table.insert(slots, { slot = num, prompt = v })
                            break
                        end
                    end
                end
    ...
    -- Dispara o prompt de venda
    local function sell(prompt)
        pcall(function() fireproximityprompt(prompt) end)
    end

    -- Vende todos os slots
    local function sellAll()
        for _, data in pairs(getSlots()) do
            sell(data.prompt)
            task.wait(0.1)
        end
    end

    -- Vende um slot aleatório
    local function randomSell()
        local slots = getSlots()
        if #slots > 0 then
            sell(slots[math.random(1, #slots)].prompt)
        end
    end

    -- Remove GUI antiga se existir
    local old = PlayerGui:FindFirstChild("PR1VAutoSell")
    if old then old:Destroy() end

    -- Cria a ScreenGui seguindo o estilo do PR1VHub
    local sellGui = Instance.new("ScreenGui")
    sellGui.Name = "PR1VAutoSell"
    sellGui.ResetOnSpawn = false
    sellGui.Parent = PlayerGui

    local sellFrame = Instance.new("Frame")
    local mobileScale = IS_MOBILE and 0.75 or 1
    sellFrame.Size = UDim2.new(0, 200 * mobileScale, 0, 300 * mobileScale)
    local sellPos = (Config.Positions and Config.Positions.AutoSell) or { X = 0.75, Y = 0.55 }
    sellFrame.Position = UDim2.new(sellPos.X, 0, sellPos.Y, 0)
    sellFrame.BackgroundColor3 = Theme.Background
    sellFrame.BackgroundTransparency = 0.38
    sellFrame.BorderSizePixel = 0
    sellFrame.ClipsDescendants = true
    sellFrame.Parent = sellGui

    ApplyViewportUIScale(sellFrame, 200, 300, 0.5, 0.9)
    Instance.new("UICorner", sellFrame).CornerRadius = UDim.new(0, 12)
    local sellStroke = Instance.new("UIStroke", sellFrame)
    sellStroke.Color = Theme.Accent2
    sellStroke.Thickness = 1.5
    sellStroke.Transparency = 0.15
    CreateGradient(sellStroke)

    -- Header / drag handle
    local sellHeader = Instance.new("Frame", sellFrame)
    sellHeader.Size = UDim2.new(1, 0, 0, 40)
    sellHeader.BackgroundTransparency = 1
    MakeDraggable(sellHeader, sellFrame, "AutoSell")

    local sellTitle = Instance.new("TextLabel", sellHeader)
    sellTitle.Size = UDim2.new(1, -20, 0, 22)
    sellTitle.Position = UDim2.new(0, 12, 0, 6)
    sellTitle.BackgroundTransparency = 1
    sellTitle.Text = "HAPPY HUB PRIVATE"
    sellTitle.Font = Enum.Font.GothamBlack
    sellTitle.TextSize = 14
    sellTitle.TextColor3 = Theme.TextPrimary
    sellTitle.TextXAlignment = Enum.TextXAlignment.Left

    local sellSub = Instance.new("TextLabel", sellHeader)
    sellSub.Size = UDim2.new(1, -20, 0, 14)
    sellSub.Position = UDim2.new(0, 12, 0, 24)
    sellSub.BackgroundTransparency = 1
    sellSub.Text = "/ Auto Sell"
    sellSub.Font = Enum.Font.GothamMedium
    sellSub.TextSize = 10
    sellSub.TextColor3 = Theme.TextSecondary
    sellSub.TextXAlignment = Enum.TextXAlignment.Left

    -- Botões ALL e RND
    local function makeTopBtn(text, xScale)
        local btn = Instance.new("TextButton", sellFrame)
        btn.Size = UDim2.new(0.44, 0, 0, 28)
        btn.Position = UDim2.new(xScale, 0, 0, 42)
        btn.BackgroundColor3 = Theme.SurfaceHighlight
    ...
    local allBtn = makeTopBtn("SELL ALL", 0.04)
    local rndBtn = makeTopBtn("RANDOM", 0.52)

    allBtn.MouseButton1Click:Connect(function()
        allBtn.BackgroundColor3 = Theme.Success
        allBtn.TextColor3 = Color3.new(0, 0, 0)
        task.spawn(function()
            sellAll()
            task.wait(0.4)
            allBtn.BackgroundColor3 = Theme.SurfaceHighlight
            allBtn.TextColor3 = Theme.TextPrimary
        end)
        ShowNotification("AUTO SELL", "Vendendo todos os slots...")
    end)

    rndBtn.MouseButton1Click:Connect(function()
        rndBtn.BackgroundColor3 = Theme.Accent1
        rndBtn.TextColor3 = Color3.new(0, 0, 0)
        task.spawn(function()
            randomSell()
            task.wait(0.3)
            rndBtn.BackgroundColor3 = Theme.SurfaceHighlight
            rndBtn.TextColor3 = Theme.TextPrimary
        end)
        ShowNotification("AUTO SELL", "Slot aleatório vendido")
    end)

    -- Label de seção
    local slotsLabel = Instance.new("TextLabel", sellFrame)
    slotsLabel.Size = UDim2.new(1, -20, 0, 16)
    slotsLabel.Position = UDim2.new(0, 12, 0, 76)
    slotsLabel.BackgroundTransparency = 1
    slotsLabel.Text = "SLOTS DISPONÍVEIS"
    slotsLabel.Font = Enum.Font.GothamBold
    slotsLabel.TextSize = 10
    slotsLabel.TextColor3 = Theme.TextSecondary
    slotsLabel.TextXAlignment = Enum.TextXAlignment.Left

    -- ScrollingFrame para os botões de slot
    local scroll = Instance.new("ScrollingFrame", sellFrame)
    ...
    local slotLayout = Instance.new("UIListLayout", scroll)
    ...
    local function refresh()
        ...
        for _, data in pairs(slots) do
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                ShowNotification("AUTO SELL", "Slot " .. data.slot .. " vendido")
            end)
        end
        scroll.CanvasSize = UDim2.new(0, 0, 0, #slots * 32)
        slotsLabel.Text = "SLOTS (" .. #slots .. ")"
    end

    -- Registra posição no Config
    if not Config.Positions.AutoSell then
        Config.Positions.AutoSell = { X = 0.75, Y = 0.55 }
    end

    -- Adiciona toggle no settings (linha simples)
    task.spawn(function()
        task.wait(2)
        -- Entrada no Settings GUI para esconder/mostrar o painel
        local sg = PlayerGui:FindFirstChild("SettingsUI")
        if sg then
            local sList_ = sg:FindFirstChild("sFrame") and sg.sFrame:FindFirstChild("sList")
                        or sg:FindFirstChildOfClass("Frame") and sg:FindFirstChildOfClass("Frame"):FindFirstChild("sList")
            -- fallback: procura o scroll dentro do settings
            if not sList_ then
                for _, v in pairs(sg:GetDescendants()) do
                    if v:IsA("ScrollingFrame") and v.Name == "sList" then sList_ = v; break end
                end
            end
        end
    end)
    ...
    refresh()
    ShowNotification("AUTO SELL", "Módulo carregado ✓")
end)

-- AUTO BUY — simples e otimizado
task.spawn(function()
    local autoBuyActive = (Config.AutoBuyEnabled == true)

    -- UI
    local _old = PlayerGui:FindFirstChild("PR1VAutoBuyUI")
    if _old then _old:Destroy() end

    local abGui = Instance.new("ScreenGui")
    abGui.Name = "PR1VAutoBuyUI"; abGui.ResetOnSpawn = false; abGui.DisplayOrder = 30
    abGui.Parent = PlayerGui

    local abFrame = Instance.new("Frame", abGui)
    abFrame.Size = UDim2.new(0, 220, 0, 90)
    local _p = (Config.Positions and Config.Positions.AutoBuy) or {X=0.75, Y=0.02}
    abFrame.Position = UDim2.new(_p.X, 0, _p.Y, 0)
    abFrame.BackgroundColor3 = Color3.fromRGB(32, 16, 60)
    abFrame.BackgroundTransparency = 0.08
    ...
    abStroke.Color = Theme.Accent1; abStroke.Thickness = 1.5; abStroke.Transparency = 0.35

    -- Header + drag
    local abHdr = Instance.new("Frame", abFrame)
    abHdr.Size = UDim2.new(1, 0, 0, 36); abHdr.BackgroundTransparency = 1
    MakeDraggable(abHdr, abFrame, "AutoBuy")

    local abTitle = Instance.new("TextLabel", abHdr)
    abTitle.Size = UDim2.new(1, -12, 1, 0); abTitle.Position = UDim2.new(0, 12, 0, 0)
    abTitle.BackgroundTransparency = 1; abTitle.Text = "AUTO BUY"
    abTitle.Font = Enum.Font.GothamBlack; abTitle.TextSize = 15
    abTitle.TextColor3 = Theme.Accent1; abTitle.TextXAlignment = Enum.TextXAlignment.Left

    -- Divider
    local abDiv = Instance.new("Frame", abFrame)
    abDiv.Size = UDim2.new(1, -20, 0, 1); abDiv.Position = UDim2.new(0, 10, 0, 36)
    abDiv.BackgroundColor3 = Theme.Accent1; abDiv.BackgroundTransparency = 0.6
    abDiv.BorderSizePixel = 0

    -- Row de controles
    local abRow = Instance.new("Frame", abFrame)
    abRow.Size = UDim2.new(1, -16, 0, 40); abRow.Position = UDim2.new(0, 8, 0, 44)
    abRow.BackgroundColor3 = Theme.Surface; abRow.BackgroundTransparency = 0.05
    abRow.BorderSizePixel = 0
    Instance.new("UICorner", abRow).CornerRadius = UDim.new(0, 8)

    -- Toggle button
    local abToggleBtn = Instance.new("TextButton", abRow)
    abToggleBtn.Size = UDim2.new(0, 90, 0, 28); abToggleBtn.Position = UDim2.new(0, 4, 0.5, -14)
    abToggleBtn.Font = Enum.Font.GothamBlack; abToggleBtn.TextSize = 11
    abToggleBtn.AutoButtonColor = false; abToggleBtn.BorderSizePixel = 0
    Instance.new("UICorner", abToggleBtn).CornerRadius = UDim.new(0, 6)

    -- Keybind button
    local abKeyBtn = Instance.new("TextButton", abRow)
    abKeyBtn.Size = UDim2.new(0, 46, 0, 24); abKeyBtn.Position = UDim2.new(1, -100, 0.5, -12)
    abKeyBtn.BackgroundColor3 = Theme.SurfaceHighlight; abKeyBtn.Text = Config.AutoBuyKey or "K"
    abKeyBtn.Font = Enum.Font.GothamBold; abKeyBtn.TextSize = 11
    abKeyBtn.TextColor3 = Theme.Accent1; abKeyBtn.AutoButtonColor = false; abKeyBtn.BorderSizePixel = 0
    Instance.new("UICorner", abKeyBtn).CornerRadius = UDim.new(0, 5)

    -- Range label (simples, sem slider — usa o valor do Config)
    local abRangeLbl = Instance.new("TextLabel", abRow)
    abRangeLbl.Size = UDim2.new(0, 48, 1, 0); abRangeLbl.Position = UDim2.new(1, -52, 0, 0)
    abRangeLbl.BackgroundTransparency = 1; abRangeLbl.Font = Enum.Font.GothamMedium
    abRangeLbl.TextSize = 10; abRangeLbl.TextColor3 = Theme.TextSecondary
    abRangeLbl.TextXAlignment = Enum.TextXAlignment.Center

    local function refreshUI()
        if autoBuyActive then
            abToggleBtn.Text = "ON ●"; abToggleBtn.BackgroundColor3 = Theme.Accent1
            abToggleBtn.TextColor3 = Color3.new(0,0,0)
        else
            abToggleBtn.Text = "OFF ○"; abToggleBtn.BackgroundColor3 = Theme.SurfaceHighlight
            abToggleBtn.TextColor3 = Theme.TextSecondary
        end
        abRangeLbl.Text = (Config.AutoBuyRange or 17) .. " st"
    end
    refreshUI()

    if not Config.Positions then Config.Positions = {} end
    if not Config.Positions.AutoBuy then Config.Positions.AutoBuy = {X=0.75, Y=0.02} end

    -- Ring visual
    local abRing = nil
    local function createRing()
        pcall(function()
            local ex = Workspace:FindFirstChild("PR1VAutoBuyRing"); if ex then ex:Destroy() end
            local r = Instance.new("Part")
            r.Name = "PR1VAutoBuyRing"; r.Shape = Enum.PartType.Cylinder
            r.Anchored = true; r.CanCollide = false; r.CanTouch = false
            r.CanQuery = false; r.CastShadow = false
            r.Material = Enum.Material.Neon; r.Transparency = 0.55; r.Color = Theme.Accent1
            local range = Config.AutoBuyRange or 17
            r.Size = Vector3.new(0.5, range*2, range*2); r.Parent = Workspace; abRing = r
        end)
    end
    local function destroyRing()
        if abRing then pcall(function() abRing:Destroy() end); abRing = nil end
        local ex = Workspace:FindFirstChild("PR1VAutoBuyRing")
        if ex then pcall(function() ex:Destroy() end) end
    end

    RunService.Heartbeat:Connect(function()
        if not autoBuyActive or not abRing or not abRing.Parent then return end
        local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        if not hrp then return end
        local range = Config.AutoBuyRange or 17
    ...
    LocalPlayer.CharacterAdded:Connect(function()
        task.wait(0.5); if autoBuyActive then createRing() end
    end)
        ...
        task.spawn(function()
            for _ = 1, 15 do
                ...
                pcall(function()
                    local char = LocalPlayer.Character
                    local hum = char and char:FindFirstChildOfClass("Humanoid")
                    if hum and not char:FindFirstChild(toolName) then
                        local t = LocalPlayer.Backpack:FindFirstChild(toolName)
    ...
    local function toggleAutoBuy()
        ...
        Config.AutoBuyEnabled = autoBuyActive; SaveConfig(); refreshUI()
        if autoBuyActive then createRing(); startCarpetLock(); ShowNotification("AUTO BUY","✅ ATIVADO")
        else destroyRing(); stopCarpetLock(); ShowNotification("AUTO BUY","❌ DESATIVADO") end
    end
    ...
    abKeyBtn.MouseButton1Click:Connect(function()
        ...
        local c; c = UserInputService.InputBegan:Connect(function(inp)
            if inp.UserInputType == Enum.UserInputType.Keyboard then
                ...
                abKeyBtn.Text = inp.KeyCode.Name; abKeyBtn.TextColor3 = Theme.Accent1
                SaveConfig(); refreshUI(); c:Disconnect()
            end
        end)
    end)
    ...
    -- Scan simples (sem UI de lista, sem gen value, só o necessário)
    local function scanConveyor()
        ...
        for _, obj in ipairs(Workspace:GetDescendants()) do
            ...
            if not (tx == "purchase" or tx:find("purchase") or tx:find("comprar")) then continue end
            local pp = obj.Parent; if not pp then continue end
            local rp = pp:IsA("Attachment") and pp.Parent or pp
            if not (rp and rp:IsA("BasePart")) then continue end
            table.insert(results, {prompt=obj, part=rp})
        end
        return results
    end
    ...
    -- BodyPosition hover
    local bodyPos = nil
    local lockedPart, lockedPrompt = nil, nil
    ...
    -- Hover loop
    RunService.Heartbeat:Connect(function()
        if not autoBuyActive or not lockedPart or not lockedPart.Parent then destroyBP(); return end
        local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        if not hrp then destroyBP(); return end
        ensureBP(hrp).Position = lockedPart.Position + Vector3.new(0, 5, 0)
    end)
    ...
    task.spawn(function()
        while true do
            task.wait(0.08)
            if not autoBuyActive or not lockedPart or not lockedPart.Parent then continue end
            if lockedPrompt and lockedPrompt.Parent and lockedPrompt.Enabled then
    ...
    task.spawn(function()
        while true do
            ...
            if not autoBuyActive then
                lockedPart=nil; lockedPrompt=nil; stopCarpetLock(); destroyBP(); continue
            end
            if lockedPart and not lockedPart.Parent then
                lockedPart=nil; lockedPrompt=nil
                ShowNotification("AUTO BUY","📦 Buscando próximo...")
            end
            if lockedPart then continue end
            local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            if not hrp then continue end
            local radius = Config.AutoBuyRange or 17
            local ok, animals = pcall(scanConveyor)
            if not ok or not animals then continue end
            ...
            if best then
                lockedPart=best.part; lockedPrompt=best.prompt
                startCarpetLock()
                ShowNotification("AUTO BUY","🔒 Comprando...")
            end
        end
    end)

    ShowNotification("AUTO BUY","Módulo carregado ✓")
end)
